...
percentiles = joblib.load('percentiles.joblib')
...
if option == "Manual Input":
    ...
    input_df['tenure_category'] = pd.cut(input_df['tenure'], bins=percentiles, labels=['Low', 'Medium', 'High', 'Very High'], include_lowest=True)
    ...
    if st.button("Predict"):
        ...
        input_transformed_df = pd.DataFrame(input_transformed, columns=transformer.get_feature_names_out())

        # Filter features based on importance
        input_filtered = input_transformed_df[important_features]

        # Make predictions
        predictions = best_model.predict(input_filtered)
    ...
    if uploaded_file is not None:
        ...
        if not all(column in csv_df.columns for column in required_columns):
            ...
            # Apply the same feature engineering steps as during training
            csv_df['tenure_category'] = pd.cut(csv_df['tenure'], bins=percentiles, labels=['Low', 'Medium', 'High', 'Very High'], include_lowest=True)
            bins = [20, 35, 50, float('inf')]
            labels = ['20-35', '35-50', '50+']
            ...
            input_transformed = transformer.transform(csv_df)
            input_transformed_df = pd.DataFrame(input_transformed, columns=transformer.get_feature_names_out())
